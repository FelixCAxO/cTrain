...
Required fields are `schemaVersion`, `id`, `version`, `title`, `description`, `language`, `difficulty`, `estimatedSeconds`, `tags`, `prerequisites`, and `targetCode`. Optional fields are `$schema`, `learningGoals`, `defects`, `completionChecks`, and `languageVersion`; `languageVersion` is required for difficulty 3+ lessons. Authored lesson JSON supports `language: "java"`, `language: "c"`, and `language: "python"`. Supported versions are `C89`, `C99`, `C11`, `C17`, `C23`; `Java 8`, `Java 9`, `Java 11`, `Java 14`, `Java 15`, `Java 16`, `Java 17`, `Java 21`, `Java 22`, `Java 23`, `Java 24`, `Java 25`, `Java 26`; and `Python 3.10`, `Python 3.11`, `Python 3.12`, `Python 3.13`, `Python 3.14`. Current-file practice is separate from authored lesson JSON and still preserves the VS Code language id from the active editor.
...
The script reads `docs/roadmap-coverage.tsv` by default and prints JSON objects shaped as `{ "track": "...", "roadmapNode": "...", "coveredBy": ["lesson-id"], "examReadyBy": ["lesson-id"], "minExamReadyLessons": 1, "minCompletionChecksPerLesson": 2 }`. Rows with non-zero readiness columns require non-preview lessons with enough `completionChecks`, so Java 26 side-track features do not count as Oracle Java SE 25 (1Z0-831) exam-ready coverage. `docs/oracle-1z0-831-objective-map.md` reconciles Oracle's published objective areas and sub-objectives against the mock-exam blueprint plus roadmap rows. Mock exams draw 50 Java certification-focused questions from that completion-check bank, excluding preview, Java 26, HTTP client side-track lessons, and non-Java lessons by default; they keep a 120-minute timer, sample by objective, persist objective-level results, and show missed-question explanations.
